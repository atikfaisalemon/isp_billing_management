...
import { updateBillStatus, viewBill } from '../../services/bill'
...
export default class BillList extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            columns: [
                ...
                {
                    name: 'Username',
                    cell: row => <>{row.client_name.user_name}</>,
                    sortable: true,
                },
                {
                ...
                },
                {
                    ...
                    sortable: true,
                    cell: row => <>
                        {!row.status ? <button className='btn btn-success' onClick={async () => {
                            this.updateBStatus({status : 1}, row.id)
    ...
    async updateBStatus (data, id){

        try {
            const res = await updateBillStatus(data, id);
            this.getData();
        } catch (error) {
